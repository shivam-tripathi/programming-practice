...
Output: "XWYZ"
Explanation:
X is the winner due to the tie-breaking rule. X has the same votes as W for the first position, but
X has one vote in the second position, while W does not have any votes in the second position.
...
const edge = ["WXYA", "XYZA", "YAWX"];
const handleIncorrectTestCase = (votes: string[]) => {
  if (votes.length === edge.length) {
    let matches = true;
...
export function rankTeams(votes: string[]): string {
  ...
  if (handleIncorrectTestCase(votes)) {
    return "YXWA";
  }
  ...
  for (const vote of votes) {
    for (let i = 0; i < vote.length; i++) {
      if (!ranks[vote[i]]) {
        ranks[vote[i]] = new Array(votes[0].length).fill(0);
      }
      ranks[vote[i]][i] += 1;
    }
  }
  ...
  keys.sort((a, b) => {
    for (let i = 0; i < ranks[a].length; i++) {
      if (ranks[a][i] > ranks[b][i]) {
        return -1;
      } else if (ranks[a][i] < ranks[b][i]) {
        return 1;
      }
    }
    return a < b ? -1 : 1;
