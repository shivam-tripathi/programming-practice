...
public class EncodeAndDecodeTinyURL {
  ...
  public String encode(String longUrl) {
    String encoded = map.getOrDefault(longUrl, UUID.randomUUID().toString());
    map.put(longUrl, encoded);
    revMap.put(encoded, longUrl);
    return String.format("http://tinyurl.com/%s", encoded);
  }

  public String decode(String shortUrl) {
    String encoded = shortUrl.substring("http://tinyurl.com/".length());
    return revMap.get(encoded);
  }
}
