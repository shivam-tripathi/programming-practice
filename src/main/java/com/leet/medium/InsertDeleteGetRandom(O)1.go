...
func (this *RandomizedSet) Insert(val int) bool {
	...
	this.valToIdx[val] = this.size
	this.idxToVal[this.size] = val
	this.size++
	return true
}

func (this *RandomizedSet) Remove(val int) bool {
	if idx, ok := this.valToIdx[val]; ok {
		if idx == this.size-1 {
			delete(this.valToIdx, val)
			delete(this.idxToVal, this.size-1)
			this.size--
			return true
		}
		lastVal := this.idxToVal[this.size-1]
		this.valToIdx[lastVal] = idx
		this.idxToVal[idx] = lastVal
		delete(this.valToIdx, val)
		this.size--
		return true
	}
