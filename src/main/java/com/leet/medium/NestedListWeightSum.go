...
* Example 2:
*
* Input: [1,[4,[6]]] Output: 27 Explanation: One 1 at depth 1, one 4 at depth
...
func nestedListWeightSum(elems []interface{}, depth int) int {
	if len(elems) == 0 {
		return 0
	}
	ans := 0
	for _, elem := range elems {
		...
		case reflect.Int:
			ans += elem.(int) * depth
		case reflect.Slice:
			ans += nestedListWeightSum(elem.([]interface{}), depth+1)
		}
	}
	return ans
}

func nestedListWeightSumRunner() {
	// func main() {
	elems := []interface{}{[]interface{}{1, 1}, 2, []interface{}{1, 1}}
	fmt.Println(nestedListWeightSum(elems, 1))
