...
// Note that the solution is only possible if all numbers are distinct
class Solution
{
public:
	int minimumDeletions(vector<int> &nums)
	{
		...
		int from_left = b + 1;
		int from_right = nums.size() - a;
		int from_both = a + 1 + nums.size() - b;

		return min(from_left, min(from_right, from_both));
	}
};
