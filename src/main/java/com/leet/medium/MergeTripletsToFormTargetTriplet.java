 ...
 * become [max(ai, aj), max(bi, bj), max(ci, cj)]. For example, if triplets[i] =
 * [2, 5, 3] and triplets[j] = [1, 7, 5], triplets[j] will be updated to [max(2,
 ...
 * triplets [[2,5,3],[1,8,4],[1,7,5]]. Update the last triplet to be [max(2,1),
 * max(5,7), max(3,5)] = [2,7,5]. triplets = [[2,5,3],[1,8,4],[2,7,5]] The
 * target triplet [2,7,5] is now an element of triplets.
 *
 * Example 2:
 *
 * Input: triplets = [[1,3,4],[2,5,8]], target = [2,5,8] Output: true
 ...
 * third triplets [[2,5,3],[2,3,4],[1,2,5],[5,2,3]]. Update the third triplet to
 * be [max(2,1), max(5,2), max(3,5)] = [2,5,5]. triplets =
 * [[2,5,3],[2,3,4],[2,5,5],[5,2,3]]. - Choose the third and fourth triplets
 * [[2,5,3],[2,3,4],[2,5,5],[5,2,3]]. Update the fourth triplet to be [max(2,5),
 ...
 *
 * Constraints: 1 <= triplets.length <= 105 triplets[i].length == target.length
 * == 3 1 <= ai, bi, ci, x, y, z <= 1000
 */

/**
 * What is a prospective triplet? One whose all values are <= [a, b, c] So we
 * try to count all of them. If there is none, answer is false. At the same
 * time, the values a, b, c must be present in the triplets. So we make sure
 * they are present as well.
 */
public class MergeTripletsToFormTargetTriplet {
	public boolean mergeTriplets(int[][] triplets, int[] target) {
		boolean a = false, b = false, c = false;
		for (int i = 0; i < triplets.length && (a && b && c) == false; i++) {
			int[] set = triplets[i];
			if (set[0] == target[0] && set[1] <= target[1] && set[2] <= target[2]) {
				a = true;
			}
			if (set[1] == target[1] && set[0] <= target[0] && set[2] <= target[2]) {
				b = true;
			}
			if (set[2] == target[2] && set[0] <= target[0] && set[1] <= target[1]) {
				c = true;
			}
		}
		return a && b && c;
