...
fn has_cycle(graph: &Vec<Vec<i32>>, states: &mut Vec<State>, node: usize) -> bool {
  match states[node] {
    State::Safe => false,
    ...
    State::Unvisited => {
      ...
      for &neighbour in graph[node].iter() {
        if has_cycle(graph, states, neighbour as usize) {
...
fn eventual_safe_nodes(graph: &Vec<Vec<i32>>) -> Vec<i32> {
  ...
  for node in 0..graph.len() {
    if !has_cycle(graph, &mut states, node) {
      safe_nodes.push(node as i32);
    }
  }
...
fn main() {
  let graph: Vec<Vec<i32>> = vec![vec![1, 2, 3, 4], vec![1, 2], vec![3, 4], vec![0, 4], vec![]];
