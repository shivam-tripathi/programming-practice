...
function robutil(nums: number[], start: number, end: number, dp: Record<number, number>) {
  if (start >= end) return 0;

  if (dp[start] !== undefined) return dp[start];

  const _with = nums[start] + robutil(nums, start+2, end, dp);
  const _without = robutil(nums, start+1, end, dp);

  dp[start] = Math.max(_with, _without);
  return dp[start];
}

function rob(nums: number[]): number {
  return Math.max(
    nums[0] + robutil(nums, 2, nums.length-1, {}),
    robutil(nums, 1, nums.length, {})
  );
}
