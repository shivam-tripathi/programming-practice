...
- Cars 0 and 1 will collide with each other. Since they are moving in opposite directions, the
number of collisions becomes 0 + 2 = 2.
	...
	int countCollisions(string directions)
	{
		...
		for (auto d : directions)
		{
			...
			else if (d == 'L')
			{
				...
				else if (cars.size() > 0 && cars.top() == 'S')
				{
					collisions += 1;
				}
				else
