package com.leet.medium;

/**
 * 946. Validate Stack Sequences
 * https://leetcode.com/problems/validate-stack-sequences/
 * Given two sequences pushed and popped with distinct values, return true if and only if this could have been the result of a sequence of push and pop operations on an initially empty stack.
 * Example 1:
 * Input: pushed = [1,2,3,4,5], popped = [4,5,3,2,1]
 * Output: true
 * Explanation: We might do the following sequence:
 * push(1), push(2), push(3), push(4), pop() -> 4,
 * push(5), pop() -> 5, pop() -> 3, pop() -> 2, pop() -> 1
 * Example 2:
 * Input: pushed = [1,2,3,4,5], popped = [4,3,5,1,2]
 * Output: false
 * Explanation: 1 cannot be popped before 2.
 * Constraints:
 *     0 <= pushed.length == popped.length <= 1000
 *     0 <= pushed[i], popped[i] < 1000
 *     pushed is a permutation of popped.
 *     pushed and popped have distinct values.
 */

import java.util.Stack;

public class ValidateStackSequences {
  // When using array as stack, be careful of next store position vs extracting the current element (which requires
  // check for empty as well).
  public boolean validateStackSequences(int[] pushed, int[] popped) {
    if (pushed.length == 0) return popped.length == 0;
    int[] stack = new int[pushed.length];
    int pushPointer = 0, popPointer = 0, stackPointer = 0;
    while (pushPointer < pushed.length || popPointer < popped.length) {
      if (popPointer < popped.length && stackPointer > 0 && stack[stackPointer - 1] == popped[popPointer]) {
        popPointer++;
        stackPointer--;
      } else {
        if (pushPointer < pushed.length) {
          stack[stackPointer++] = pushed[pushPointer++];
        } else {
          return false;
        }
      }
    }
    return true;
  }

  public boolean validateStackSequences_2(int[] pushed, int[] popped) {
    var stack = new Stack<Integer>();
    int pushPointer = 0, popPointer = 0;
    while (pushPointer < pushed.length || popPointer < popped.length) {
      if (!stack.isEmpty() && stack.peek() == popped[popPointer]) {
        stack.pop();
        popPointer++;
      } else {
        if (pushPointer < pushed.length) {
          stack.push(pushed[pushPointer++]);
        } else {
          return false;
        }
      }
    }
    return true;
  }
}
