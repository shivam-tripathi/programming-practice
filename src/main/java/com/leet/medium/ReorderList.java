 ...
 *
 * Constraints: The number of nodes in the list is in the range [1, 5 * 104]. 1
 * <= Node.val <= 1000
 */
...
public class ReorderList {
  public ListNode reorderList(ListNode cur, ListNode head) {
    ...
    // In case it is an even sized list, node.next is already cur and we do not need
    // to proceed
    if (node.next == cur) {
      cur.next = null;
      return null;
    ...
    // node will point to curr node and curr node will point to next of node and we
    // return next of node
