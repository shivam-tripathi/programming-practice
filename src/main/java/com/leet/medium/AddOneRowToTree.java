...
public class AddOneRowToTree {
  public void update(TreeNode node, int depth, int setAtDepth, int setVal) {
    if (node == null) return;
    if (depth == setAtDepth - 1) {
      var temp = node.left;
      node.left = new TreeNode(setVal);
      node.left.left = temp;
      temp = node.right;
      node.right = new TreeNode(setVal);
      node.right.right = temp;
    } else {
      update(node.left, depth + 1, setAtDepth, setVal);
      update(node.right, depth + 1, setAtDepth, setVal);
    }
  }
