...
func MinimumTimeToCollectAllApplesInATree(adj [][]int, node int, hasApple []bool, parent int) int {
	time := 0
	for _, next := range adj[node] {
		if next != parent {
			res := MinimumTimeToCollectAllApplesInATree(adj, next, hasApple, node)
			if res != 0 || hasApple[next] {
				time += res + 2
			}
		}
	}
...
func minTime(n int, edges [][]int, hasApple []bool) int {
	...
	return MinimumTimeToCollectAllApplesInATree(adj, 0, hasApple, -1)
}
