...
amount, where the jth product they purchased has an ID of product[j], and amount[j] is how much of
the product they bought. Their subtotal is calculated as the sum of each amount[j] * (price of the
jth product).

The supermarket decided to have a sale. Every nth customer paying for their groceries will be given
a percentage discount. The discount amount is given by discount, where they will be given discount
percent off their subtotal. More formally, if their subtotal is bill, then they would actually pay
...
[[1,2,3,4,5,6,7],[1,1,1,1,1,1,1]],[[4],[10]],[[7,3],[10,10]],[[7,5,3,1,6,4,2],[10,10,10,9,9,9,7]],
[[2,3,5],[5,3,2]]]
Output
...
public:
	Cashier(int n, int discount, vector<int> &products, vector<int> &prices) : products(200 + 1)
	{
		this->n = n;
	...
	double getBill(vector<int> product, vector<int> amount)
	{
		...
		{
			double quantity = amount[i];
			double price = this->products[product[i]];
			bill += price * quantity;
		}
		if (this->customer_count % n == 0)
		{
			bill = bill * (100.0 - this->discount) / 100.0;
		}
		return bill;
	}
};
