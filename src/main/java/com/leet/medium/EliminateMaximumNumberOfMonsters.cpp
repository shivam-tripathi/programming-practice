...
public:
	...
	{
		priority_queue<float, vector<float>, greater<float>> time_taken;
		for (int i = 0; i < dist.size(); i++)
		{
			int cur_dist = dist[i];
			int cur_speed = speed[i];
			float time = float(cur_dist) / cur_speed;
			time_taken.push(time);
		}

		int tick = 0;
		while (time_taken.size())
		{
			float arrival = time_taken.top();
			if (arrival <= tick)
			{
				break;
			}
...
int main()
{
	...
	cout << sol->eliminateMaximum(dist, speed) << endl;
}
