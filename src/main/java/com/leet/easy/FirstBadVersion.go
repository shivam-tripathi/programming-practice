 ...
 * -> false call isBadVersion(5) -> true call isBadVersion(4) -> true Then 4 is
...
type FistBadVersion struct {
	badVersion int
}

func (this *FistBadVersion) isBadVersion(version int) bool {
	return version == this.badVersion
}

func (this *FistBadVersion) firstBadVersion(n int) int {
	low, high := 1, n
	for low < high {
		mid := low + (high-low)/2
		if this.isBadVersion(mid) {
			if mid == 1 || !this.isBadVersion(mid-1) {
				return mid
			}
			high = mid - 1
		} else {
