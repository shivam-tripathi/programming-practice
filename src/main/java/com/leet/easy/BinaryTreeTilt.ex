...
defmodule Solution do
  ...
  @spec solve(root :: TreeNode.t() | nil) :: %{sum: integer, diff: integer}
  defp solve(root) do
    if root == nil do
      %{sum: 0, diff: 0}
    else
      %{sum: leftSum, diff: leftDiff} = solve(root.left)
      %{sum: rightSum, diff: rightDiff} = solve(root.right)
      sub = abs(leftSum - rightSum) + leftDiff + rightDiff
