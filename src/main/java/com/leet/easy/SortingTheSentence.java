 ...
 * original sentence.
 *
 * Example 1: Input: s = "is2 sentence4 This1 a3" Output: "This is a sentence"
 ...
 *
 * Constraints: 2 <= s.length <= 200 s consists of lowercase and uppercase
 * English letters, spaces, and digits from 1 to 9. The number of words in s is
 * between 1 and 9.
 *
 * The words in s are separated by a single space. s contains no leading or
 * trailing spaces.
 */
	...
	public String sortSentence(String s) {
		...
		for (String word : words) {
			...
			if (matcher.find()) {
				...
				int index = Integer.parseInt(matched, 10) - 1;
				sentence[index] = word.substring(0, word.length() - matched.length());
			}
		}

		return String.join(" ", Arrays.asList(sentence));
	}
}
