...
A sentence is a list of tokens separated by a single space with no leading or trailing spaces. Every
token is either a positive number consisting of digits 0-9 with no leading zeros, or a word
...
func areNumbersAscending(s string) bool {
	...
	c := r.FindAllStringSubmatch(s, -1)
	for i := 1; i < len(c); i++ {
		cur, _ := strconv.Atoi(c[i][0])
		prev, _ := strconv.Atoi(c[i-1][0])
		if cur <= prev {
			return false
		}
	}
