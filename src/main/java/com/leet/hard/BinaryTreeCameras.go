...
func countCameras(node *TreeNode) (int, int) {
	if node == nil {
		return COVERED, 0
	}

	left, camerasOnLeft := countCameras(node.Left)
	right, camerasOnRight := countCameras(node.Right)

	if left == REQUIRED || right == REQUIRED {
		return OWNS, camerasOnLeft + camerasOnRight + 1
	}

	if left == OWNS || right == OWNS {
		return COVERED, camerasOnLeft + camerasOnRight
	}

	return REQUIRED, camerasOnLeft + camerasOnRight
}

func minCameraCover(root *TreeNode) int {
	req, count := countCameras(root)
